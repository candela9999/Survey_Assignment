...
#### Modelling relationship with DK
This may be unnecessary as we have shown above low correlation with NA responses.
Testing if we run a logistic regression to test for the most important/significant variables.  

Make a subset of data with only our variables above +/- 9.5% correlation and above 10% cramers V.
...
Now model with those variables to see if they are significant in explaining the DK values. 

Overall, the logistic model is not very interpretable. We run a stepwise AIC on thesse most correlated variables and then use the best model to test the overall model fit.  

If we want to do this really proper, we should probably oversample the NA share due to class imbalance and model with training and test sets. But I think this regression is fine. 
...
dk_fit <- glm(target_NA ~ lgb_discri + age + I(age*age) + antilgbtq_rights + internet_use + social_class + country_name, 
              data = dk_target, 
              family = "binomial")
...
Some things we can see with the odds ratios (excluding interpretation of countries as that is covered above)... 
... Can't really interpret (all internet users and non-users were more likely to be NA with the exception of 2-3 times a month use... that's useless info and not logical). 
For social class, those who did not respond, or were more likely say other social class were more likely to NA respond too. 

Overall, there is generally a low correlation of the NA values with 


Below is an option using undersampling which is probably more legit too. maybe good to include if we aren't doing in other sections. 
...
#confusionMatrix(predict(rfunder, test), test$target_NA, positive = '1')
...
Use Cramers V since we only have factors
